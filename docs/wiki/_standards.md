...
- ****Q1**: `.unwrap()` in production**
  - Status: ✅ Fixed June 23 | Domain: indexer, cli
  - 9 sites fixed (UUID gen, mutex lock, treasury key) — all safe patterns
...
- ****S3**: KRC-20 KCC-20 output ownership validation (ICC pattern)**
  - Status: ✅ Fixed June 23 | Domain: contracts
  - Added `validateKcc20Input()` helper with `readInputStateWithTemplate` + `OpCovInputIdx`
...
- ****Q2/Q3**: Magic number `200` hardcoded in 3 covenant files — no single source of truth**
  - Status: ✅ Fixed June 23 | Domain: contracts, indexer, cli, wasm-sdk
  - `FEE_DENOMINATOR` changed from `u64` to `i64` for type consistency
