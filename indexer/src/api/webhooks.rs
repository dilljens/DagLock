...
pub async fn create(
    ...
    // Validate event name
    let valid_events = [
        "escrow.created", "escrow.settled", "escrow.refunded",
        "escrow.disputed", "escrow.cancelled", "escrow.expired",
        "offer.created", "offer.accepted",
    ];
    if !valid_events.contains(&body.event.as_str()) {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(json!(ApiError::new("invalid_event", format!("Unknown event: {}. Valid: {}", body.event, valid_events.join(", "))))),
        ));
    }

    let id = format!("wh_{}", Uuid::new_v4().to_string().split('-').next().unwrap());
    ...
    .bind(&id).bind(&app_id).bind(&body.event).bind(&body.url).bind(now)
    .execute(&state.db)
    .await
    .map_err(|_e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!(ApiError::new("internal_error", "Failed to create webhook"))),
        )
    })?;
...
pub async fn list(
    ...
    let hooks: Vec<Value> = rows.into_iter().map(|r| json!({
...
pub async fn delete(
    ...
    let result = sqlx::query("DELETE FROM webhooks WHERE id = ?1 AND app_id = ?2")
        .bind(&hook_id).bind(&app_id)
        .execute(&state.db)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!(ApiError::new("internal_error", "Failed to delete webhook"))),
            )
        })?;
    ...
    if result.rows_affected() > 0 {
        ...
        Err((StatusCode::NOT_FOUND, Json(json!(ApiError::new("webhook_not_found", "Webhook not found")))))
    }
}
