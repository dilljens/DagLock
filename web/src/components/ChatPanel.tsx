...
export function ChatPanel({ escrow, onMutated }: ChatPanelProps) {
	...
	const counterpartyPubkey = isBuyer ? escrow.chat_pubkey_seller : escrow.chat_pubkey_buyer;
	const mySubmittedPubkey = isBuyer ? escrow.chat_pubkey_buyer : escrow.chat_pubkey_seller;
	...
	useEffect(() => {
		...
		try {
			const secret = deriveSharedSecret(keypair.secret, decodeBase64(counterpartyPubkey));
			...
			if (m.content_enc && m.nonce) {
				...
				if (plain !== null) {
					if (m.chat_sig && m.seq && keypair) {
						const senderPubkey =
							m.sender_address === escrow.buyer_address
	...
	async function handleSend(e: React.FormEvent) {
		...
		try {
			...
			const chatSig = signChatMessage(keypair.secret, enc.ciphertext, enc.nonce, escrow.id, seq);
	...
	return (
		<div className="panel" style={{ marginTop: "12px" }}>
			...
			>
				<strong style={{ fontSize: "14px" }}>💬 Encrypted Chat</strong>
				<div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
					{!myPubkeySubmitted && (
						<span style={{ fontSize: "11px", color: "#ff9800" }}>⏳ Submitting chat key…</span>
					)}
					...
					{canChat && <span style={{ fontSize: "11px", color: "#4caf50" }}>✓ E2E encrypted</span>}
					{messages.length > 0 && (
						<button
				...
				<>
					...
					>
						...
						{!loading && messages.length === 0 && (
							<p
								className="muted"
								style={{ textAlign: "center", fontSize: "12px", padding: "16px 0" }}
							>
