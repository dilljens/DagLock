...
export function deriveSharedSecret(mySeed: Uint8Array, theirEdPubkey: Uint8Array): Uint8Array {
	const myX = clampX25519Secret(mySeed);
	const theirX = ed25519ToX25519Public(theirEdPubkey);
	return nacl.scalarMult(myX, theirX);
}

export function encryptMessage(sharedSecret: Uint8Array, plaintext: string): EncryptedMessage {
