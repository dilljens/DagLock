...
export async function signMessage(message: string, type: "ecdsa" | "schnorr" = "schnorr"): Promise<string> {
  if (!window.kasware) {
...
export function subscribeToWallet(onStateChange: (state: WalletState) => void): () => void {
  ...
  const onAccountsChanged = (accounts: string[]) => {
    if (accounts.length === 0) {
      onStateChange({ detected: true, connected: false, address: null, network: null, balance: null, loading: false, error: null });
    }
  };
  const onNetworkChanged = (network: string) => onStateChange({ detected: true, connected: false, address: null, network, balance: null, loading: false, error: null });
  const onDisconnect = () => {
    onStateChange({ detected: true, connected: false, address: null, network: null, balance: null, loading: false, error: null });
  };
